...
export default function SwapRoute() {
  ...
  let handleSubmit = async (e) => {
    ...
    let data = { 'start': e.target.start.value, 'dest': e.target.dest.value, 'via': e.target.via.value, 'stime': e.target.stime.value, 'rtime': e.target.rtime.value }
     let response = await fetch(`http://127.0.0.1:8000/route/change/${bid}/`, {
        method: 'POST',
        headers: {
      ...
      <main className={styles.main}>
        <div className="container position-absolute top-50 start-50 translate-middle">
          <form className={`${stl.viewbox} card card-body mx-auto box-shadow`} onSubmit={handleSubmit} autoComplete='true'>
            ...
            {result ? <div className="submit text-center mb-2"><button type="submit" name="submit"  className={`${stl.blackbtn} btn`}>Update</button><Link href={`/busdb/seatprice/${bid}`} className={`${stl.blackbtn} btn`}>Next</Link></div> : <div className="submit text-center mb-2"><button type="submit" name="submit" className={`${stl.blackbtn} btn`}>Save</button></div>}
          </form>
        </div>
      </main>
    </>
